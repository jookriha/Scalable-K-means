function [outputs]= skmeanspar(inputDir,K,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Parallel SK-means||
%
%   inputs:
...
for run =1:runs
    ...
    spmd
        ...
        for ir = 1:r
            ra = rand(N,1);
            Cp = gopsubset(@vertcat,X(l*D/psi>ra,:),WNsi);
            Cps = [Cps; Cp];
...
if(openParPool)
    ...
    %
    % Parallel subset K-means
    %
    function bestCInd = parsubkmeans
    warning off all;
    spmd
        if(WNsi(1) == labindex)
            C = Cw;
            labBroadcastSubset(WNsi(1),WNsi,C);
        else
            C = labBroadcastSubset(WNsi(1),WNsi);
        end
        iters = 0; L = ones(N,1);
